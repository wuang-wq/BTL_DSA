...
public:
    ...
    void assign(initializer_list<T> ilist); // gán giá trị từ 1 danh sách khởi tạo vào vector và cập nhật kích thước của vector
...
private:
    T* curr; // con trỏ để theo dõi vị trí hiện tại của vector
public:
...
//constructor
//gán giá trị value cho các phần tử trong vector
template <typename T>
...
inline void Vector<T>::push_back(const T& val) {
    ...
    /*cấp phát 1 bộ nhớ nhỏ = 8, đủ để chứa data */
...
/* Gán giá trị nhất định cho tất cả các phần tử trong Vector 
    và cập nhật kích thước của Vector.*/
template <typename T>
...
/*Gán giá trị từ một mảng con trỏ vào Vector 
    và cập nhật kích thước của Vector*/
...
/*Gán giá trị từ một danh sách khởi tạo vào Vector 
    và cập nhật kích thước của Vector.*/
template <typename T>
...
// /*  hàm này chèn phần tử vào vị trí chỉ định*/
// template <typename T>
...
//element access
/*trả về tham chiếu tới một giá trị chỉ định*/
template <typename T>
T& Vector<T>::at(int position) {
    return Elements[position];
}
/*trả về tham chiếu hằng tới một giá trị chỉ định*/
template <typename T>
const T& Vector<T>::at(int position) const {
    return Elements[position];
}
/*trả về tham chiếu tới giá trị đầu tiên của vector*/
template <typename T>
T& Vector<T>::front() {
    return Elements[0];
}
...
/*trả về tham chiếu tới giá trị cuối cùng của vector*/
template <typename T>
T& Vector<T>::back() {
...
/*đọc và ghi giá trị của phần tử thứ i*/
template <typename T>
...
/*truy cập trực tiếp đến dãy dữ liệu của vector*/
template <typename T>
T* Vector<T>::data() {
    return Elements;
}
